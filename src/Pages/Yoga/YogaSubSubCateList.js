...
const YogaSubSubCateList = ({navigation,route}) => {
  ...
  return (
     <View style={styles.body}>
        ...
        <SafeAreaView>
            <Header iconName={'menu'} title={data.vSubYogaName} />
        </SafeAreaView>
                    ...
                    <View style={styles.imageRows}>
                        {
                            YogaSubCategoryList.map((curEle,index)=>{
                                return <TouchableOpacity activeOpacity={.9} onPress={()=>{navigation.navigate(RoutName.YOGA_MAIN_CATEGORY_LIST,{data:curEle})}} style={styles.column1} key={index}>
                                            <Text style={styles.boxText}>{curEle.vSubSubYogaName}</Text>
...
export default YogaSubSubCateList
